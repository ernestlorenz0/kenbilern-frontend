...
export default function SlideEditor({ initialSlides, selectedTemplate, onBack, previewMode }) {
  ...
  const handleExportPDF = async (customFilename = '') => {
    ...
    const originalSlideIndex = current;
    ...
    const getSlideContainerForSlide = async (slideIndex) => {
      ...
      // Wait for the slide to render (increased for complex slides with images)
    ...
    // Restore original slide after export
    setCurrent(originalSlideIndex);
  ...
  const handleShare = useCallback(async () => {
    try {
      ...
      const slideshowId = await saveSharedSlideshow(slideshowData);
