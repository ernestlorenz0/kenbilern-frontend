...
export default function HistoryPage() {
  ...
  const createTempSlideRenderer = async (slide, templateName, slideIndex) => {
    return new Promise((resolve) => {
      ...
      // Render the themed slide properly (mirror editor/preview logic)
      const renderThemedSlide = (slide, theme, index) => {
        ...
        if (index === 0 && theme.TitleSlide) {
          return React.createElement(theme.TitleSlide, {
            title: components.find(c => c.type === "title")?.content || "",
            subtitle: components.find(c => c.type === "paragraph")?.content || "",
            imageUrl: components.find(c => c.type === "image" && !c.isDraggable)?.content || ""
          });
        }
        ...
        if (components.some(c => c.type === "toc") && theme.TOCSlide) {
          ...
          const tocSections = components
            ...
            .slice(0, 6)
            .map((item, idx) => ({
              id: item.id || `toc-item-${idx}`,
              title: sanitizeTitle(item.content),
              page: item.pageNumber || (idx + 3),
              subsections: item.subsections || [],
              ...(item.subtitle ? { subtitle: item.subtitle } : {})
            }));
          const tocData = { title: tocTitle, sections: tocSections, ...(tocComponent.data || {}) };
          return React.createElement(theme.TOCSlide, { tocData });
        }
        ...
        // Content slides
        const rawTitle = components.find(c => c.type === "title")?.content || "";
        const rawTitle2 = components.find(c => c.type === "title2")?.content || "";
        const title = formatTermTitle(rawTitle);
        const content = components.find(c => c.type === "paragraph")?.content || "";
        const title2 = formatTermTitle(rawTitle2);
        const content2 = components.find(c => c.type === "paragraph2")?.content || "";
        const imageUrl = components.find(c => c.type === "image" && !c.isDraggable)?.content || "";
        ...
        if (slide.layout && theme[slide.layout]) {
          ...
          return React.createElement(LayoutComp, { title, content, title2, content2, imageUrl, autoGeneratedImage });
        }

        // Fallbacks similar to live editor
        if (theme.ImageSlide && imageUrl) {
          return React.createElement(theme.ImageSlide, { title, imageUrl });
        }
        if (theme.MainSlide) {
          return React.createElement(theme.MainSlide, { title, content, autoGeneratedImage });
        }
        if (theme.MainSlide1) {
          return React.createElement(theme.MainSlide1, { title, content, autoGeneratedImage });
        }
        if (theme.MainSlide2) {
          return React.createElement(theme.MainSlide2, { title, content, title2, content2, imageUrl });
        }
        if (theme.MainSlide3) {
          return React.createElement(theme.MainSlide3, { title, points: [content] });
        }
        if (theme.MainSlide4) {
          return React.createElement(theme.MainSlide4, { title, content, title2, content2, imageUrl });
        }
        if (theme.MainSlide5) {
          return React.createElement(theme.MainSlide5, { title, content, imageUrl });
        }
        if (theme.MainSlide6) {
          return React.createElement(theme.MainSlide6, { title, content });
        }
        if (theme.ContentSlide) {
          return React.createElement(theme.ContentSlide, { title, content });
        }
        if (theme.ContentSlideText) {
          return React.createElement(theme.ContentSlideText, { title, content });
        }
