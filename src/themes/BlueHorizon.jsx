...
import BookloverPng from '../pngs/booklover.png';
import BookloverSvg from '../svgs/booklover.svg';
  ...
  return (
    <section className="relative w-[1920px] h-[1080px] bg-[#2b6cb0] overflow-hidden flex items-center justify-center">
      ...
      <div className="relative z-10 text-center">
        <h1 className="text-white font-[HedvigLettersSerif] text-7xl">{title}</h1>
...
export function TOCSlide({ tocData }) {
  ...
  return (
    <section className="relative w-[1920px] h-[1080px] bg-gradient-to-b from-sky-700 via-sky-500 to-sky-300 text-white flex flex-col items-center justify-center overflow-hidden">
      ...
      {/* Title */}
      <h2 className="text-6xl font-extrabold mb-16 tracking-wide text-white drop-shadow-[0_0_20px_rgba(135,206,250,0.6)]">
        {title}
      </h2>
  ...
  return (
    <section className="relative w-[1920px] h-[1080px] bg-[#ebf8ff] overflow-hidden flex flex-col items-center justify-center p-10 text-center">
      ...
      <div className="relative z-10 max-w-3xl">
        ...
        <p className="text-gray-800 text-[2.7rem] leading-relaxed">{content}</p>
      </div>
    </section>
  );
}
...
export function MainSlide1({ title, content, autoGeneratedImage }) {
  return (
    <section className="relative w-[1920px] h-[1080px] bg-[#ebf8ff] overflow-hidden flex">
      ...
      {/* Right: text */}
      <div className="w-1/2 flex flex-col justify-center p-12 text-right mr-10">
...
export function MainSlide2({ title, content, title2, content2 }) {
  return (
    <section className="relative w-[1920px] h-[1080px] bg-[#ebf8ff] overflow-hidden flex flex-col items-center justify-center">
      {/* Top Half: First Term */}
      <div className="absolute top-[100px] left-0 right-0 h-[420px] px-20 z-10">
        <div className="h-full bg-white rounded-2xl border-8 border-sky-500/60 shadow-2xl p-10 flex flex-col justify-center">
          <h2 className="font-[HedvigLettersSerif] text-6xl text-[#2b6cb0] mb-6">{title || 'Term 1'}</h2>
          <div className="h-[4px] w-24 bg-[#f6ad55] rounded-full mb-6"></div>
          <p className="text-gray-800 text-[2.5rem] leading-relaxed">{content || 'Definition for term 1'}</p>
        </div>
      </div>

      {/* Bottom Half: Second Term */}
      <div className="absolute bottom-[100px] left-0 right-0 h-[420px] px-20 z-10">
        <div className="h-full bg-white rounded-2xl border-8 border-blue-500/60 shadow-2xl p-10 flex flex-col justify-center">
          <h2 className="font-[HedvigLettersSerif] text-6xl text-[#3182ce] mb-6">{title2 || 'Term 2'}</h2>
          <div className="h-[4px] w-24 bg-[#3182ce] rounded-full mb-6"></div>
          <p className="text-gray-800 text-[2.5rem] leading-relaxed">{content2 || 'Definition for term 2'}</p>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="relative w-[1920px] h-[1080px] bg-[#ebf8ff] overflow-hidden flex">
      <div className="w-1/2 flex items-center justify-center p-4">
        <img src={BookloverSvg} alt="booklover" className="w-[700px]" crossOrigin="anonymous" onError={(e)=>{ e.currentTarget.onerror=null; e.currentTarget.src=BookloverPng; }} />
      </div>
      <div className="w-1/2 flex flex-col justify-center p-10 text-right mr-10">
        <h2 className="font-[HedvigLettersSerif] text-7xl text-[#2b6cb0] mb-4">{title}</h2>
        <p className="text-gray-800 text-[2.7rem] leading-relaxed">{content}</p>
      </div>
    </section>
...
export function MainSlide4({ title, content, title2, content2 }) {
  return (
    <section className="relative w-[1920px] h-[1080px] flex items-center justify-center overflow-hidden bg-[#ebf8ff]">
      {/* Left: First Term */}
      <div className="absolute left-[100px] top-1/2 -translate-y-1/2 w-[800px] z-10">
        <div className="bg-[#2b6cb0] rounded-2xl shadow-2xl p-10 text-white">
          <h2 className="font-[HedvigLettersSerif] text-6xl font-bold mb-6">{title || 'Term 1'}</h2>
          <div className="h-[4px] w-24 bg-[#f6ad55] rounded-full mb-6"></div>
          <p className="text-[2.5rem] leading-relaxed">{content || 'Definition for term 1'}</p>
        </div>
      </div>

      {/* Right: Second Term */}
      <div className="absolute right-[100px] top-1/2 -translate-y-1/2 w-[800px] z-10">
        <div className="bg-white rounded-2xl border-4 border-[#3182ce] shadow-2xl p-10">
          <h2 className="font-[HedvigLettersSerif] text-6xl font-bold text-[#2b6cb0] mb-6">{title2 || 'Term 2'}</h2>
          <div className="h-[4px] w-24 bg-[#3182ce] rounded-full mb-6"></div>
  ...
  return (
    <section className="relative w-[1920px] h-[1080px] bg-gradient-to-tr from-[#bee3f8] to-[#ebf8ff] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center max-w-4xl">
        <h2 className="font-[HedvigLettersSerif] text-7xl text-[#2b6cb0] mb-6">
          {title}
        </h2>
