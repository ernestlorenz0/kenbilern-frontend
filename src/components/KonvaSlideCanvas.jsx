...
export default function KonvaSlideCanvas({
  ...
  const createExportContainer = () => {
    return (
      <div 
        ...
        className="fixed w-[1920px] h-[1080px] bg-white"
        style={{ 
          top: '0px',
          left: '0px',
          visibility: 'hidden',
  ...
  const renderThemedSlide = (slide, idx) => {
    ...
    const title2 = slide.components?.find(c => c.type === "title2")?.content || "";
    const content2 = slide.components?.find(c => c.type === "paragraph2")?.content || "";
    const imageUrl = slide.components?.find(c => c.type === "image" && !c.isDraggable)?.content || "";
    const autoGeneratedImage = slide.components?.find(c => c.type === "auto_generated_image")?.content || null;

    // If API assigned a layout, respect it
    if (slide.layout && Theme[slide.layout]) {
      const LayoutComp = Theme[slide.layout];
      return <LayoutComp title={title} content={content} title2={title2} content2={content2} imageUrl={imageUrl} autoGeneratedImage={autoGeneratedImage} />;
    }
